...
class Mesh
{
	friend class IBFDeviceRenderer;// Use m_pVertexBuffer and m_pIndexBuffer.
	friend class BFResourceManager;// Update m_pVertexBuffer and m_pIndexBuffer.
	friend class BFGeometry;// Create Geometry Mesh.

public:

	Mesh() : mPosition(0.f, 0.f, 0.f), mScaleX(1.f), mScaleY(1.f), mScaleZ(1.f), 
	...
	Mesh(const VEC3& _pos) : Mesh() { mPosition = _pos; };
	Mesh(const VEC3& _pos, const VEC3& _scales, const VEC3& _rotations) :
